...
# inspect whether any samples are missing one housekeeping gene
housekeeping_summary %>%
  filter(is.na(rpoB_cov) | is.na(gyrB_cov))
...
ko_expression$KO_label <- factor(
  ...
  levels = c("narG","narH","narI","napA","napB","napC",
             "nirK","nirS","norB","nosZ",
             "hzsA","hzsB")
)
...
```{r, message=FALSE, warning=FALSE}
ggplot(ko_expression,
       aes(x = sample, y = total_expression, color = KO_ID, group = KO_ID)) +
  geom_line() +
  geom_point() +
  theme_bw()
```
...
ko_order <- c(
  ...
  "K15864",  # nirS
  "K04561",  # norB
  "K02305",  # norC / napA-like in your previous setup
  "K00376"   # nosZ
)
