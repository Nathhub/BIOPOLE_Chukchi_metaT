...
# ---- load Bracken (family-level example) ----
br <- read_tsv("data/St1_S4S.bracken.F", show_col_types = FALSE) %>%
  filter(taxonomy_id != 9606) %>%                 # optional: remove human
...
br_plot <- br %>%
  ...
  mutate(
    taxon = fct_lump_n(name, n = top_n, w = rel_abund),
    taxon = fct_reorder(taxon, rel_abund, .desc = TRUE)
  ) %>%
  group_by(taxon) %>%
  summarise(rel_abund = sum(rel_abund), .groups = "drop")

# ---- named palette: top taxa get your colors, Other is grey ----
taxa_levels <- levels(br_plot$taxon)
n_levels <- length(taxa_levels)

# Ensure we have enough colors for n=20 (we do)
palette_vals <- c(my_colors[seq_len(min(top_n, length(my_colors)))], "grey80")[seq_len(n_levels)]
names(palette_vals) <- taxa_levels

ggplot(br_plot, aes(x = "St1_S4S", y = rel_abund, fill = taxon)) +
  geom_col(width = 0.7) +
