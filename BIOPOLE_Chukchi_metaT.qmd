...
# This script has been run for each Station and for denitrification and anammox data.
# We will use the resulting tables in the next code chunk
library(tidyverse)

# read function annotations
functions <- read_tsv("St3_gene_functions.tsv", show_col_types = FALSE)

# read coverages
cov <- read_tsv("St3_denitrification-GENE-COVERAGES.txt", show_col_types = FALSE)

# read detection
det <- read_tsv("St3_denitrification-GENE-DETECTION.txt", show_col_types = FALSE)
...
functions_ko_target <- functions %>%
  dplyr::filter(gene_callers_id %in% target_gene_ids) %>%
  dplyr::filter(source == "KOfam") %>%
  dplyr::filter(accession %in% c("K00370", "K00371", "K00374", "K02567","K02568", # KOs denitrif
                                 "K00368", "K15864", "K04561", "K02305", "K00376")) %>%
  dplyr::select(gene_callers_id, KO_ID = accession, `function`) %>%
  dplyr::distinct()

  # dplyr::filter(accession %in% c("K00368", "K15864", "K20932", "K20933", # KOs anammox
  #                                "K20934", "K20935")) %>%

# merge everything
final_table <- cov_long %>%
  left_join(det_long, by = c("gene_callers_id", "sample")) %>%
  left_join(functions_ko_target, by = "gene_callers_id") %>%
  select(gene_callers_id, KO_ID, `function`, sample, coverage, detection)

write_tsv(final_table, "St3_denitrification_final_table.tsv")
```

Same for the housekeeping genes:

```{r, eval=FALSE, message=FALSE, warning=FALSE}
# This script has been run for each Station.
# We will use the resulting tables in the next code chunk

# read coverages
cov <- read_tsv("St3_housekeeping-GENE-COVERAGES.txt", show_col_types = FALSE)

# read detection
det <- read_tsv("St3_housekeeping-GENE-DETECTION.txt", show_col_types = FALSE)
...
functions_hk_target <- functions %>%
  dplyr::filter(gene_callers_id %in% target_gene_ids) %>%
  dplyr::filter(source == "KOfam") %>%
  dplyr::filter(accession %in% c("K03043", "K02470")) %>%
  dplyr::select(gene_callers_id, KO_ID = accession, `function`) %>%
  dplyr::distinct()
...
final_table <- cov_long %>%
  left_join(det_long, by = c("gene_callers_id", "sample")) %>%
  left_join(functions_hk_target, by = "gene_callers_id") %>%
  select(gene_callers_id, KO_ID, `function`, sample, coverage, detection)
...
# read all tables and add metadata from filename
all_tables <- map_dfr(files, function(f) {
  read_tsv(f, show_col_types = FALSE) %>%
    mutate(
      file = basename(f),
      station = str_extract(basename(f), "^St[0-9]+"),
      pathway = case_when(
        str_detect(f, "denitrification") ~ "denitrification",
        str_detect(f, "anammox") ~ "anammox",
        str_detect(f, "housekeeping") ~ "housekeeping",
        TRUE ~ NA_character_
      )
    )
})

# split out housekeeping and target pathways
housekeeping <- all_tables %>%
  filter(pathway == "housekeeping",
         KO_ID %in% c("K03043", "K02470"))

targets <- all_tables %>%
  filter(pathway %in% c("denitrification", "anammox"))

# summarise housekeeping coverage per station x sample x KO
housekeeping_summary <- housekeeping %>%
  group_by(station, sample, KO_ID) %>%
  summarise(total_coverage = sum(coverage, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = KO_ID, values_from = total_coverage) %>%
  mutate(
    rpoB_cov = K03043,
    gyrB_cov = K02470,
    hk_geom = sqrt(rpoB_cov * gyrB_cov)
  ) %>%
  select(station, sample, rpoB_cov, gyrB_cov, hk_geom)

normalized_targets <- targets %>%
  left_join(housekeeping_summary, by = c("station", "sample")) %>%
  mutate(
    normalized_coverage = coverage / hk_geom
  )
```
...
ko_expression <- normalized_targets %>%
  group_by(station, sample, pathway, KO_ID, `function`) %>%
  summarise(
    total_expression = sum(normalized_coverage, na.rm = TRUE),
    .groups = "drop"
  )
...
ko_expression$KO_label <- ko_labels[ko_expression$KO_ID]

ko_expression$KO_label <- factor(
  ko_expression$KO_label,
  levels = c("narG","narH","narI","napA","napB",
             "nirK","nirS","norB","norC","nosZ",
             "hzsA","hzsB")
)

ggplot(ko_expression,
       aes(x = KO_label, y = total_expression, fill = pathway)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.8) +
  scale_y_log10() +
  labs(
    x = "Gene",
    y = "Normalized expression (relative to housekeeping)",
    fill = "Pathway", title = "Overall Relative Expression of Denitrification and Anammox Genes"
  ) +
...
```{r, message=FALSE, warning=FALSE}
ggplot(ko_expression,
       aes(x = sample, y = total_expression, color = KO_label, group = KO_label)) +
  geom_line() +
  geom_point() +
  theme_bw()
```
  
#### Heatmap of normalized denitrification gene expression across stations and depths.
...
heatmap_df <- ko_expression %>%
  filter(pathway == "denitrification") %>%
  select(KO_ID, sample, total_expression) %>%
  pivot_wider(names_from = sample, values_from = total_expression)
...
ko_order_present <- ko_order[ko_order %in% rownames(heatmap_matrix)]
heatmap_matrix <- heatmap_matrix[ko_order_present, , drop = FALSE]

# replace KO IDs with gene names
ko_labels <- c(
  K00370 = "narG",
  K00371 = "narH",
  K00374 = "narI",
...
rownames(heatmap_matrix) <- ko_labels[rownames(heatmap_matrix)]
...
sample_order_present <- sample_order[sample_order %in% colnames(heatmap_matrix)]
heatmap_matrix <- heatmap_matrix[, sample_order_present, drop = FALSE]
...
gene_annotation <- data.frame(
  Step = c(
    ...
    "NO reduction",
    "N2O reduction"
  )[match(ko_order_present, ko_order)]
)

rownames(gene_annotation) <- rownames(heatmap_matrix)

gene_annotation$Step <- factor(
  gene_annotation$Step,
  levels = c("Nitrate reduction", "Nitrite reduction", "NO reduction", "N2O reduction")
)
...
# breaks (include 0 explicitly)
breaks <- c(
  0,
  seq(1e-6, max_val, length.out = 99)  # small offset to separate zero
)

# color palette: first color = grey (for zero), rest = gradient
colors <- c(
  "white",  # zero values
  colorRampPalette(c("lightgrey", "yellow", "orange", "red"))(99)
)
...
rownames(number_matrix) <- rownames(heatmap_matrix)
colnames(number_matrix) <- colnames(heatmap_matrix)
...
pheatmap(
  ...
  number_color = "black",
  main = "Normalized expression of denitrification genes"
)
...
**Context and normalization approach**

To assess the relative expression of denitrification genes across samples, gene coverages derived from metatranscriptomic data were normalized against housekeeping genes. Specifically, coverage values for each gene were divided by the geometric mean of the coverage of rpoB (K03043) and gyrB (K02470), two conserved single-copy genes involved in core cellular processes. This approach accounts for differences in sequencing depth and overall transcriptional activity, allowing gene expression to be interpreted relative to baseline cellular function. Normalized coverages were then summed per KEGG Orthology (KO) identifier within each sample to estimate the total community-level expression of each enzymatic step in the denitrification pathway. The resulting values were log-transformed to facilitate comparison across genes and samples with different expression magnitudes.

**Interpretation of denitrification gene expression patterns**
The heatmap indicates that expression within the denitrification pathway is largely dominated by the nitrite reduction step, with nirK (K00368) consistently showing substantially higher normalized expression than the other denitrification genes, particularly in mid-depth samples such as St3_S14M. In contrast, genes associated with nitrate reduction (nar and nap complexes), nitric oxide reduction, and nitrous oxide reduction displayed extremely low or undetectable expression across most samples, suggesting limited activity of these downstream denitrification processes in the studied water column.

Overall, these patterns indicate that complete denitrification was likely not actively occurring at significant levels during sampling. However, the elevated expression of nirK suggests involvement in alternative nitrogen cycling pathways rather than canonical denitrification.

**NOTE**: In marine systems, nirK is also commonly associated with ammonia-oxidizing archaea, particularly members of the genus Nitrosopumilus, where it has been linked to nitrifier denitrification.

:::

### Final figure: functional classification

Let's use discret values instead of a continuous gradient of normailsed expression.

- Values around 0.1–1 → very strong expression
- Values around 0.01–0.1 → moderate 
- Values <0.001 → low but detectable 
- Values ≈0 → absent / negligible


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}
heatmap_cat <- heatmap_matrix

heatmap_cat[heatmap_matrix == 0] <- "Absent"
heatmap_cat[heatmap_matrix > 0 & heatmap_matrix < 1e-2] <- "Low"
heatmap_cat[heatmap_matrix >= 1e-2 & heatmap_matrix < 1e-1] <- "Moderate"
heatmap_cat[heatmap_matrix >= 1e-1] <- "High"
...
heatmap_num <- matrix(
  as.numeric(factor(heatmap_cat, levels = c("Absent","Low","Moderate","High"))),
  nrow = nrow(heatmap_cat),
  dimnames = dimnames(heatmap_cat)
)
...
pheatmap(
  ...
  legend_labels = c("Absent","Low","Moderate","High"),
  main = "Categorized expression of denitrification genes"
)

```


Expression levels were categorized into four classes based on normalized coverage: absent (0), low (<0.1%), moderate (10⁻²–10⁻¹), and high (>10⁻¹), allowing visualization of relative transcriptional activity across denitrification steps.

#### Heatmap of normalized anammox gene expression across stations and depths.
...
missing_kos <- setdiff(ko_order, rownames(heatmap_matrix))

if(length(missing_kos) > 0){
  zero_rows <- matrix(
    0,
    nrow = length(missing_kos),
    ncol = ncol(heatmap_matrix),
    dimnames = list(missing_kos, colnames(heatmap_matrix))
  )
  
  heatmap_matrix <- rbind(heatmap_matrix, zero_rows)
}
...
gene_annotation$Step <- factor(
  gene_annotation$Step,
  levels = c("Nitrite reduction", "Hydrazine synthesis", "Hydrazine oxidation")
)

ann_colors <- list(
  Step = c(
    "Nitrite reduction"   = "#00c98b",
    "Hydrazine synthesis" = "#8c6cff",
    "Hydrazine oxidation" = "#f28e2b"
  )
)
...
pheatmap(
  ...
  display_numbers = number_matrix,
  number_color = "black",
  main = "Normalized expression of anammox genes"
)
...
## Looking at nitrification gene expression

```{bash}
#| eval: false   
#| echo: true
NITRIFICATION_KOS='K28504|K10944|K10945|K10946|K10535|K00368|K00370|K00371'

awk -F'\t' -v kos="$NITRIFICATION_KOS" \
'($3 ~ kos){print $1}' St1_gene_functions.tsv \
| sort -u > nitrification_genes.ids

anvi-export-gene-coverage-and-detection \
-p St1_MERGED/PROFILE.db \
-c St1_contigs.db \
--genes-of-interest nitrification_genes.ids \
-O St1_nitrification
```
...
# read coverages
cov <- read_tsv("St1_nitrification-GENE-COVERAGES.txt", show_col_types = FALSE)

# read detection
det <- read_tsv("St1_nitrification-GENE-DETECTION.txt", show_col_types = FALSE)
...
functions_ko_target <- functions %>%
  dplyr::filter(gene_callers_id %in% target_gene_ids) %>%
  dplyr::filter(source == "KOfam") %>%
  dplyr::filter(accession %in% c(
    "K28504", "K10945", "K10946",  # amoA, amoB, amoC
    "K10944",                      # pmoA-amoA
    "K10535",                      # hao
    "K00368",                      # nirK
    "K00370", "K00371"             # nxrA / nxrB
  )) %>%
  dplyr::select(gene_callers_id, KO_ID = accession, `function`) %>%
  dplyr::distinct()
...
final_table <- cov_long %>%
  left_join(det_long, by = c("gene_callers_id", "sample")) %>%
  left_join(functions_ko_target, by = "gene_callers_id") %>%
  distinct(gene_callers_id, sample, KO_ID, .keep_all = TRUE) %>%
  select(gene_callers_id, KO_ID, `function`, sample, coverage, detection)

write_tsv(final_table, "St1_nitrification_final_table.tsv")
```
...
all_tables <- list.files(
  "data/",
  pattern = "_nitrification|housekeeping",
  full.names = TRUE
) %>%
  map_dfr(~ read_tsv(.x, show_col_types = FALSE) %>%
    mutate(
      file = basename(.x),
      station = str_extract(file, "^St[0-9]+"),
      pathway = str_extract(file, "nitrification|housekeeping")
    )
  )

housekeeping_summary <- all_tables %>%
  filter(pathway == "housekeeping", KO_ID %in% c("K03043", "K02470")) %>%
  group_by(station, sample, KO_ID) %>%
  summarise(coverage = sum(coverage, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = KO_ID, values_from = coverage) %>%
  transmute(
    station,
    sample,
    hk_geom = sqrt(K03043 * K02470)
  )

normalized_nitrification <- all_tables %>%
  filter(pathway == "nitrification") %>%
  left_join(housekeeping_summary, by = c("station", "sample")) %>%
  mutate(normalized_coverage = coverage / hk_geom)
```
...
normalized_nitrification <- normalized_nitrification %>%
  distinct(gene_callers_id, sample, KO_ID, .keep_all = TRUE)

# summarize normalized expression
ko_expression <- normalized_nitrification %>%
  group_by(sample, KO_ID) %>%
  summarise(total_expression = sum(normalized_coverage, na.rm = TRUE),
            .groups = "drop")
...
ko_order <- names(ko_labels)

heatmap_matrix <- ko_expression %>%
  pivot_wider(names_from = sample, values_from = total_expression) %>%
  mutate(across(-KO_ID, ~replace_na(., 0))) %>%
  column_to_rownames("KO_ID") %>%
  as.matrix()

heatmap_matrix <- heatmap_matrix[ko_order[ko_order %in% rownames(heatmap_matrix)], , drop = FALSE]
...
heatmap_matrix <- heatmap_matrix[, sample_order[sample_order %in% colnames(heatmap_matrix)], drop = FALSE]
...
ann_colors <- list(
  Step = c(
    "Ammonia oxidation"      = "#00c98b",
    "Hydroxylamine oxidation" = "#8c6cff",
    "NO production"          = "#f28e2b",
    "Nitrite oxidation"      = "#22b7e8"
  )
)
...
breaks <- c(0, seq(1e-6, max_val, length.out = 99))
...
number_matrix <- ifelse(
  heatmap_matrix == 0, "N.D.",
  ifelse(heatmap_matrix < 0.01, "<0.01",
         sprintf("%.2f", heatmap_matrix))
)
...
pheatmap(
  ...
  angle_col = 315,
  gaps_col = c(3,5),
  main = "Normalized expression of nitrification genes"
)
```

**Note**: the nxrB most abundant sequence, when blasted, is highly similar to oen from the NOB **Nitrospina sp.** Given that Nitrospina nitrite oxidising bacteria (NOB) usually carry the whole NXR complex (nxrA, nxrB and likely nxrC), the low apparent expression of nxrA might be due to Assembly fragmentation, Annotation issues, Differential transcription, Reverse operation in nitrate reducers (“nxrB-like” genes)...

```{r, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}

library(tidyverse)

plot_df <- normalized_nitrification %>%
  filter(KO_ID %in% c("K10944", "K00368")) %>%
  group_by(sample, KO_ID) %>%
  summarise(total_norm_cov = sum(normalized_coverage, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = KO_ID, values_from = total_norm_cov) %>%
  rename(
    amoA = K10944,
    nirK = K00368
  ) %>%
  mutate(
    station = str_extract(sample, "^St[0-9]+"),
    depth = case_when(
      str_detect(sample, "S$") ~ "Surface",
      str_detect(sample, "M$") ~ "Mid-water",
      str_detect(sample, "B$") ~ "Bottom",
      TRUE ~ NA_character_
    ),
    station = factor(station),
    depth = factor(depth, levels = c("Surface", "Mid-water", "Bottom"))
  )

ggplot(plot_df, aes(x = amoA, y = nirK)) +
  geom_point(aes(shape = station, colour = depth), size = 3.5, stroke = 1) +
  ...
  labs(
    x = expression("Normalised coverage of " * italic(amoA)),
    y = expression("Normalised coverage of " * italic(nirK)),
    colour = "Depth",
    shape = "Station"
  ) +
  theme_bw(base_size = 12) +
...
```{r, message=FALSE, warning=FALSE}
cor.test(plot_df$amoA, plot_df$nirK, method = "spearman")
```

amoA and nirK normalized expression were strongly positively correlated across samples (Spearman’s ρ = 0.83, p = 0.015), suggesting that nirK expression may covary with ammonia oxidation activity. However, interpretation is limited by small sample size (n = 8) and potential spatial dependence among samples.

This result suggests that nirK expression is linked to nitrifying archaea rather than classical denitrifiers

#### Additionnal questions and notes:

- How exactly do marine archaea oxidize hydroxylamine equivalents to nitrite without canonical HAO?
- NO is required for archaeal ammonia oxidation...
- Proposed archaeal pathway - Current model (simplified): NH3 → NH2OH → NO → NO2−
- Nitrosopumilus maritimus and related AOA can generate small amounts of NO and N₂O especially under low oxygen conditions.
- Hypothesis - AOA could be doing this: NH2OH + NO → N2O (enzyme remains unknown...) Probably low production though...
...
ggplot(br_plot, aes(x = sample, y = rel_abund, fill = taxon)) +
  ...
  labs(
    x = NULL,
    y = "Relative abundance of classified transcripts",
    fill = "Family",
    title = "Taxonomic composition of expressed transcripts"
  ) +
  theme_bw()
...
# All samples from all 3 stations
sample_info <- tibble(
  sample = c("1-S4S", "2-S4M", "3-S4B",
             "5-S13M", "6-S13B",
             "7-S14S", "8-S14M", "9-S14B"),
  station = c("Station 4", "Station 4", "Station 4",
              "Station 13", "Station 13",
              "Station 14", "Station 14", "Station 14"),
  depth = c("Surface", "Mid-water", "Bottom",
            "Mid-water", "Bottom",
            "Surface", "Mid-water", "Bottom")
)

# Order depths left-to-right within each station
depth_levels <- c("Surface", "Mid-water", "Bottom")
station_levels <- c("Station 4", "Station 13", "Station 14")

read_bracken <- function(sample, station, depth) {
  file.path("data", paste0(sample, ".bracken.F")) %>%
    read_tsv(show_col_types = FALSE) %>%
    filter(taxonomy_id != 9604) %>%   # remove host at family level
    mutate(
      sample = sample,
      station = station,
      depth = depth
...
br_all <- pmap_dfr(sample_info, read_bracken)
...
br_plot <- br_all %>%
  mutate(taxon = if_else(name %in% top_taxa, name, "Other")) %>%
  group_by(station, depth, taxon) %>%
  summarise(rel_abund = sum(rel_abund), .groups = "drop")

# Legend order by overall abundance across all stations
taxon_levels <- br_plot %>%
  group_by(taxon) %>%
  summarise(total = sum(rel_abund), .groups = "drop") %>%
  arrange(desc(total)) %>%
...
palette_vals <- setNames(
  c(my_colors[1:(length(taxon_levels) - 1)], "grey80"),
  c(setdiff(taxon_levels, "Other"), "Other")
)

ggplot(br_plot, aes(x = depth, y = rel_abund, fill = taxon)) +
  geom_col(width = 0.7) +
  ...
  labs(
    ...
    y = "Relative abundance of classified transcripts",
    fill = "Family",
    title = "Taxonomic composition of expressed transcripts - Top 10 families"
  ) +
  theme_bw() +
    theme(
...
#### percentage of Nitrosopumilaceae + Pelagibacteraceae

```{r, message=FALSE, warning=FALSE}
combined_dominant <- br_all %>%
  filter(name %in% c("Nitrosopumilaceae",
                     "Candidatus Pelagibacteraceae")) %>%
  group_by(sample, station, depth) %>%
  summarise(
    combined_rel_abund = sum(rel_abund),
    combined_percent = round(combined_rel_abund * 100, 1),
    .groups = "drop"
  ) %>%
...
### Investigating correlation between nirK and Nitrosopumilaceae

```{r, message=FALSE, warning=FALSE}
library(tidyverse)

nitroso_df <- br_all %>%
  filter(name == "Nitrosopumilaceae") %>%
  select(sample, new_est_reads)
...
nitroso_df2 <- nitroso_df %>%
  mutate(
    sample_key = str_extract(sample, "S[0-9]+[A-Z]$")
  )
...
corr_df <- left_join(nirk_df2, nitroso_df2, by = "sample_key")

cor.test(
  corr_df$nirK_cov,
  corr_df$new_est_reads,
  method = "spearman"
)
...
```{r, message=FALSE, warning=FALSE, fig.width=8, fig.height=5}
ggplot(corr_df, aes(new_est_reads, nirK_cov, label = sample_key)) +
  geom_point(size = 3) +
  geom_text(nudge_y = 0.15) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw() +
  labs(
    x = "Nitrosopumilaceae Bracken estimated reads",
    y = "nirK coverage, K00368"
  )
```
